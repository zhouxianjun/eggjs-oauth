    ...
    class Model {
        ...
        async getClient (clientId, clientSecret) {
            try {
                ...
                this.ctx.logger.warn(`client id => ${clientId} 授权异常`, err);
                return false;
            }
        }
        async getUser (username, password) {
            try {
                ...
                this.ctx.logger.warn(`获取用户 => ${username} 异常`, err);
                return false;
            }
        }
        async getAccessToken (bearerToken) {
            try {
                ...
                this.ctx.logger.warn(`获取token => ${bearerToken} 异常`, err);
                return false;
            }
        }
        async saveToken (token, client, user) {
            try {
                ...
                this.ctx.logger.warn(`保存token => ${token}:${client.id} 异常`, err);
                return false;
            }
        }
        ...
        async revokeAuthorizationCode (code) {
            try {
                return await this.ctx.model.AuthorizationCode.delAuthorizationCode(code.code);
            } catch (err) {
                return false;
            }
        }
